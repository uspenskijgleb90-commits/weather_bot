...
class WeatherBot:
    ...
    def _weather_code_to_description(self, code: int) -> str:
        weather_codes = {
            ...
            55: "Сильный дождь 🌧️💧",
            56: "Легкий ледяной дождь 🌧️❄️",
            57: "Сильный ледяной дождь 🌧️💧❄️",
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not bot:
        await update.message.reply_text("Бот не инициализирован. Попробуйте /start")
        return
    
    logger.info(f"Получено сообщение: {text}")
    
    # Обработка ожидания города
    if 'waiting_for_city' in context.user_data:
        ...
        city_normalized = bot.normalize_city_name(city_name)
        found_city = None
        
        # Сначала проверяем в списке городов
        for city in MAIN_CITIES:
            if bot.normalize_city_name(city) == city_normalized:
                found_city = city
                break
        
        if not found_city:
            # Пробуем геокодировать
            geocode_result = await bot.geocode_city(city_name)
            if geocode_result:
                lat, lon, offset, tz_name = geocode_result
        ...
        if user_id not in bot.user_data:
            bot.user_data[user_id] = UserSettings()
        
        bot.user_data[user_id].city = found_city
        
        # Получаем часовой пояс
        geocode_result = await bot.geocode_city(found_city)
        if geocode_result:
            lat, lon, offset, tz_name = geocode_result
            bot.user_data[user_id].timezone_offset = offset
    ...
    if text == "🌤️ Погода сейчас":
        await bot.show_weather_now(update, context)
    
    elif text == "📍 Выбрать город":
        await bot.select_city_menu(update, context)
    
    elif text == "🔍 Поиск города":
        await bot.search_city(update, context)
    
    elif text == "⏰ Уведомления":
        await bot.notifications_menu(update, context)
    
    elif text == "📋 Список городов":
        await bot.show_city_list(update, context)
    ...
    elif any(time_str in text for time_str in NOTIFICATION_TIMES):
        # Извлекаем время из текста
        for time_str in NOTIFICATION_TIMES:
            if time_str in text:
                await bot.set_notification_time(update, context, time_str)
                break
    
    # Обработка выбора города из списка
    elif text in MAIN_CITIES:
        await bot.set_city(update, context, text)
    
    # Обработка навигации в списке городов
...
async def main():
    ...
    # Сохраняем экземпляр бота в bot_data
